...
const useDefaultProjectOptions = () => {
  const { addProjectToCollection, joinTeamProject, removeUserFromProject, deleteItem } = useAPIHandlers();
  ...
  return {
    ...
    deleteProject: (project) => deleteItem({ project }).catch(handleError),
  };
};
