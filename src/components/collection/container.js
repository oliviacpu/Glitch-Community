...
const CollectionContainer = ({
  ...
  const [previewProjects, setPreviewProjects] = useState(
    sampleSize(collection.projects, 3)
  );
  useEffect(() => {
  ...
  if (showFeaturedProject && collection.featuredProjectId) {
    [[featuredProject], projects] = partition(
      collection.projects,
      p => p.id === collection.featuredProjectId
    );
  }

  const myStuffIsEnabled = useDevToggle("My Stuff");
  const canEditNameAndDescription = myStuffIsEnabled
    ? isAuthorized && !collection.isMyStuff
    : isAuthorized;
        ...
        <div>
          ...
          <div className={styles.owner}>
            <ProfileItem
              hasLink
              {...curator}
              glitchTeam={collection.glitchTeam}
            />
          </div>
          ...
          {isAuthorized && funcs.updateColor && (
            <EditCollectionColor
              update={funcs.updateColor}
            ...
            >
              ...
              {displayHint && (
                <div className={styles.hintBody}>
                  ...
                  <Text>
                    <Emoji name="keyboard" />  Focus on a project and press space
                    to select. Move it with the arrow keys, and press space
                    again to save.
                  </Text>
                </div>
              )}
        ...
        {!collectionHasProjects && isAuthorized && (
          <div className={styles.emptyCollectionHint}>
            <Image
              ...
              height=""
            />
            <Text className={isDarkColor(collection.coverColor) && styles.dark}>
